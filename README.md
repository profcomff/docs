...
### Запуск через Docker
Запустите команду `docker compose up -d`, сервис соберется и поднимется

Результат будет находится по адресу: `http://localhost:9037`, через минуту-две после ввода команды

Прочекать состояние сервиса можно, введя `docker compose logs`, там должно быть, если сервис поднялся, такое:
```
